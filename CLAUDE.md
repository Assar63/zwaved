...
- `HostApi` codec for FUNC_ID_ZW_ADD_NODE_TO_NETWORK (`0x4A`, modes `0x01`/`0x05`/`0x06`/`0x08`/`0x09`), FUNC_ID_ZW_REMOVE_NODE_FROM_NETWORK (`0x4B`, modes `0x01`/`0x05`), and FUNC_ID_ZW_SEND_DATA (`0x13`) for application-layer commands addressed to a specific node
...
`src/message-bus/MessageBus` is the in-process publish/subscribe bus and the **only** coupling between the dongle monitor and the protocol thread: the monitor publishes `DongleStatus` events (connected with TTY path / disconnected) and the protocol thread subscribes to them, blocking on its own condition variable until a path arrives. State events like `DongleStatus` are retained, so a subscriber that comes up after the publisher already announced its current state still sees it on subscribe. Implementation is a thin wrapper around the header-only **eventpp** library (resolved via `find_package(eventpp REQUIRED)` — expected from system / cross-sysroot, not vendored); eventpp does not appear in the public header so the backing library is swappable. Dispatch is synchronous under the bus mutex; handlers must not call `publish()` or `subscribe()` reentrantly.
