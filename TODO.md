...
- [ ] [Compile the bus core once into a static library](https://github.com/Assar63/zwaved/issues/140) — `MessageBus.gen.cpp` is a 245-line source that emits a ~25 MB object (203 eventpp `subscribe`/`publish` instantiations) and is recompiled in ~9 test exes + the daemon; under the parallel Docker build that OOM-killed the gnu CI job (worked around by #138's `--parallel 2`). Build `MessageBus.cpp` + `MessageBus.gen.cpp` (+ `Logger.cpp`) once into a `zwaved_core` static lib that the daemon and all test exes link — removes the OOM at the root and cuts build time; then consider relaxing the `--parallel 2` cap.
