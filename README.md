...
- **External D-Bus interface (sdbus-c++)**: System bus name `com.tiunda.ZWaved`, methods `AddNode` / `StopAddNode` / `RemoveNode` / `StopRemoveNode` / `SetSwitchBinary` / `GetNodes`, signals `NodeInclusionStatus` / `NodeExclusionStatus` / `DongleStatus` / `SendDataStatus` / `ApplicationCommand` / `SwitchBinaryReport`. See [MANUAL.md](MANUAL.md) for operator usage.
...
- **Methods:** `AddNode(y y y ay ay)`, `StopAddNode(y)`, `RemoveNode(y y y)`, `StopRemoveNode(y)`, `SetSwitchBinary(y b y)`, `GetNodes() → a(yyyyay)`
