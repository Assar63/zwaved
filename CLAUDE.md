...
CI lives in `.github/workflows/build.yml` and runs `docker build` against the repo's `Dockerfile` on every push / PR — the Dockerfile is the single source of truth for the build environment (Ubuntu 24.04 + GCC 15 from the toolchain PPA + sdbus-c++ 2.x and eventpp from source). The CI job stops at the Dockerfile's `build` stage, which runs `ctest`, so a broken commit fails CI the same way it fails a developer's local `docker build .`. On `v*.*.*` tags an extra `publish-image` job builds the full Dockerfile and pushes to `ghcr.io/<owner>/<repo>:<version>` (and `:latest`) under the GITHUB_TOKEN.
