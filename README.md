...
- **Manifest-driven interface surface**: `InterfaceManifest.yml` at the repo root is the single source of truth for every `MessageBus` event, the full D-Bus method/signal API, and the per-CC wire constants. `scripts/codegen/` generates the matching C++ at build time (event structs and instantiations, D-Bus method/signal registrations, per-CC constants headers and simple encoders). Adding a new bus event / D-Bus method / signal / CC is "edit the manifest, rebuild" — see [scripts/codegen/README.md](scripts/codegen/README.md) for the design.
