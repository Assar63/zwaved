...
// encodeSet / encodeGet bodies live in MultilevelSwitch.gen.cpp.
...
auto MultilevelSwitch::decodeReport(std::span<const uint8_t> payload) -> std::optional<Report>
{
    if (payload.size() < REPORT_V1_BYTES || payload[0] != COMMAND_CLASS || payload[1] != SWITCH_MULTILEVEL_REPORT)
    {
        return std::nullopt;
    }
    Report out;
